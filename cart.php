...
<head>
    ...
    <!-- Page Title -->
    <title>Shopping cart - HTML 5 Template</title>
    <!-- Favicon Icon -->
...
if (isset($_POST['updatecart'])) {
    ...
    // Loop through the new quantities and update the cart items
    for ($i = 0; $i < count($qtys); $i++) {
        $newAmount = trim(addslashes($amounts[$i]));
        $qtyy = trim(addslashes($qtys[$i]));
        $newAmount = $qtyy * $newAmount;
        $id = trim(addslashes($ids[$i]));
        
        
        // Update table data
        $sql = "UPDATE cat SET qty='$qtyy' WHERE id='$id'";
        mysqli_query($conn, $sql) or die(mysqli_error($conn));
    }

    // Check if the update was successful
    if (mysqli_affected_rows($conn) > 0) {
        echo "<script>alert('Quantity Updated Successfully'); location.href='cart.php'</script>";
    } else {
        echo "<script>alert('Quantity Update Not Successful'); location.href='cart.php'</script>";
    }
}
?>
    ...
    <?php
        ...
        $sql = "SELECT * FROM cat WHERE ip = '$ip' AND user_id = '-1'";
        $result = mysqli_query($conn, $sql);

        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_array($result)) {

				// SUM(or_aprice * or_qty) AS total_price
				$qty = $row['qty'];
				$price = $row['price'];
				$subtotal = $qty * $price; 
...
?>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class=" d-none d-md-block col-md-4">
                                            <div class="table-img"><img src="dashboard/passport/<?php echo $row['passport']; ?>" alt="">
                                            </div>
                                        </div>
                                        <div class="col-12 col-sm-12 col-md-8">
                                            <h5><?php echo $row['food']; ?></h5>
                                        </div>
                                    </div>
                                </td>
                               ...
                               <!-- data-th="Quantity"-->
                                    <input type="number" class="form-control text-center" name="qty[]" value="<?php echo $row['qty'];?>">
                ...
                <input type="hidden" readonly name="id[]" value="<?php echo $row['id']; ?>">
                                ...
                                <td>
                                    ...
                                    <span class="remove"><a href="deletecart.php?id=<?php echo $row['id'];?>"><i class="fas fa-plus"></i></a></span>
                                </td>
                            </tr>
                            <!-- row 2 -->
...
<?php }} ?>
                ...
                <div class="col-lg-6 col-md-12">
                        <div class="upper-t-right d-flex justify-content-between">
                            <!-- <a class="btn" href="#">clear all</a> -->
                            <button type="submit" name="updatecart" class="btn">Update Cart</button>
                        </div>
                    </div>
                </form>
                <!-- lower table -->
...
$today = date("Y-m-d");
$ip= $_SERVER['REMOTE_ADDR'];
$sql = "SELECT SUM(price * qty) AS price FROM cat WHERE ip = '$ip' AND user_id = '-1'";
$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_array($result)) {
    $price= $row['price'];
...
?>
                                ...
                                <button class="btn" type="submit" name="submit"  > <a href="checkout.php">Process to checkout</a></button>
                            </div>
                        </div>
...
let result=Number (price) + Number (quality);
